...
export default function TelaNovaSenha() {
  ...
  if (!token) {
    Swal.fire({
      ...
      text: "Link de recuperação inválido ou expirado",
      icon: "error"
    }).then(() => {
      navigate("/login");
    });
  ...
  const enviarForms = async (e) => {
    ...
    try {
      await axios.post("/api-user/auth/reset-password", {
        token,
        senha
      });
      ...
      Swal.fire({
        ...
        text: error.response?.data?.message || "Link pode ter expirado",
        icon: "error"
      });
    }
  };
