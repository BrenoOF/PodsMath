  ...
  create: async ({ titulo, categorias_idcategorias, imagens_idimagens }) => {
    ...
    try {
      ...
      if (imagens_idimagens) {
        ...
        const [result] = await connection.query(
          'INSERT INTO temas (titulo, categorias_idcategorias) VALUES (?, ?)',
          [titulo, categorias_idcategorias]
        );
        await connection.commit();
        return { idtemas: result.insertId, titulo, categorias_idcategorias, imagens_idimagens: null };
      }
    } catch (error) {
      if (connection) await connection.rollback();
      throw error;
  ...
  update: async (id, { titulo, categorias_idcategorias, imagens_idimagens }) => {
    ...
    try {
      ...
      if (imagens_idimagens !== undefined) {
        ...
        const [result] = await connection.query(
          'UPDATE temas SET titulo = ?, categorias_idcategorias = ? WHERE idtemas = ?',
          [titulo, categorias_idcategorias, id]
        );
        await connection.commit();
        return result.affectedRows > 0;
      }
